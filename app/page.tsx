...
import { Bug, Plus, Calendar, Activity, Weight } from 'lucide-react';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function fetchBeetles() {
      const { data, error } = await supabase.from('beetles').select('*').order('id', { ascending: false });
  ...
  return (
    <div className="min-h-screen bg-[#0d160b] text-[#e2e8df] p-4 sm:p-6 pb-20 font-sans">
      ...
      <header className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 mb-8 pt-4">
        ...
        <Link href="/register" className="w-full sm:w-auto bg-[#436e32] hover:bg-[#5b8c43] text-white px-5 py-3 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg transition-all active:scale-95">
          <Plus size={18} /> 新規個体登録
        </Link>
      </header>
