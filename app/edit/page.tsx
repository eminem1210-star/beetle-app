  ...
  useEffect(() => {
    async function fetchBeetle() {
      ...
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('id', id)
        .eq('user_id', session.user.id) // 所有者チェック
        .single();
      ...
      <div className="bg-[#142011] p-5 rounded-2xl border border-[#2d4424] space-y-4">
        ...
        <div className="pt-3 border-t border-[#2d4424] space-y-3">
          ...
          <div className="flex gap-2">
            <input type="text" placeholder="メモ" value={logText} onChange={(e) => setLogText(e.target.value)} className="flex-1 bg-[#0a1108] border border-[#2d4424] p-2 rounded-lg text-sm text-[#e2e8df]" />
